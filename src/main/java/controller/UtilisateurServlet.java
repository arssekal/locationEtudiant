...
public class UtilisateurServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        String path = request.getPathInfo(); // /login, /inscription, /dashboard, /deconnexion, /profil
        ...
        if (path == null || path.equals("/") || path.equals("/login")) {
            ...
            request.getRequestDispatcher("/WEB-INF/jsp/tableau_de_bord.jsp").forward(request, response);
        }
    ...
    private void traiterInscription(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (erreurs.isEmpty()) {
            ...
            response.sendRedirect(request.getContextPath() + "/utilisateur/login?success=1");
        } else {
